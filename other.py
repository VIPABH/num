...
async def ai_handler(event):
    ...
    x = event.text
    ignore_phrases = ["مخفي اعفطلة", "مخفي اعفطله", "مخفي قيده", "مخفي قيدة", "مخفي طكة زيج", "مخفي اطلع", "مخفي غادر", "مخفي نزلني", "مخفي نزلة", "مخفي نزله", "مخفي اختار", "مخفي اختار"]
...
async def handle_whisper(event):
    ...
    m1 = await event.reply(
        f'همسة جارية الانشاء من ( [{name}](tg://user?id={sender_id}) ) إلى ( [{to_name}](tg://user?id={rid}) ) 🙂🙂',
        buttons=[button]
    )
    l[sender_id] = True
